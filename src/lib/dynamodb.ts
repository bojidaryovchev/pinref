...
export const createBookmark = async (bookmarkData: BookmarkData) => {
  ...
  await createSearchIndexEntries(
    bookmarkData.userId,
    bookmarkData.id,
    searchTokens
  );
...
export const updateBookmark = async (
  ...
  Object.entries(updates).forEach(([key, value]) => {
    if (value !== undefined && key !== 'searchTokens') { // Don't store searchTokens in the main item
...
export const searchBookmarks = async (userId: string, searchTokens: string[]): Promise<BookmarkWithScore[]> => {
  ...
  for (const token of queryTokens) {
    ...
    try {
      ...
      const bookmarkIds = new Set<string>(
        (result.Items || [])
          .map(item => item.bookmarkId as string)
          .filter(Boolean)
      );
  ...
  for (let i = 0; i < sortedIds.length; i += BATCH_SIZE) {
    ...
    const batchResults = await Promise.all(
      batch.map(id => getBookmarkById(id))
    );
